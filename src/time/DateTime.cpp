...
namespace nfx::time
{
	...
	std::int32_t DateTime::hour() const noexcept
	{
		...
		timeComponents( hour, minute, second, millisecond );
	...
	std::string DateTime::toString( Format format ) const
	{
		...
		dateComponents( y, mon, d );
		timeComponents( h, min, s, ms );
	...
	void DateTime::dateComponents( std::int32_t& year, std::int32_t& month, std::int32_t& day ) const noexcept
	{
	...
	void DateTime::timeComponents( std::int32_t& hour, std::int32_t& minute, std::int32_t& second, std::int32_t& millisecond ) const noexcept
	{
		std::int64_t timeTicks = m_ticks % constants::datetime::TICKS_PER_DAY;
	...
	TimeSpan DateTimeOffset::systemTimezoneOffset( const DateTime& dateTime ) noexcept
	{
		...
		if ( !NFX_CORE_GMTIME_R( &localTime, &utcTm ) || !NFX_CORE_LOCALTIME_R( &localTime, &localTm ) )
		{
